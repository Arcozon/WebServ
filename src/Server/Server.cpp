...
void Server::initSockets()
{
	for (size_t i = 0; i < _ports.size(); i++)
	{
		...
		if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)) == -1)
			throw std::runtime_error("bind() syscall failed to listen on port: " + std::to_string(_ports[i]));

		if (listen(fd, SOMAXCONN) == -1)
			throw std::runtime_error("listen() syscall failed to accept incoming connections on port: " + std::to_string(_ports[i]));
...
Server::Server(void) : _ports{8080, 8181, 8282}
{
	initSockets();
	initEpoll();
}
